...
class PDFStatementParser:
    ...
    def __init__(self):
        ...
        # Account number patterns for generic extraction
        self.ACCOUNT_NUMBER_PATTERNS = {
            "credit_card": r"\b(?:4[0-9]{12}(?:[0-9]{3})?|(?:5[1-5][0-9]{2}|222[1-9]|22[3-9][0-9]|2[3-6][0-9]{2}|27[01][0-9]|2720)[0-9]{12})\b",  # Visa/Mastercard
            "clabe": r"\b(?:[0-9]{18})\b",  # Mexican CLABE
            "debit_card": r"\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14})\b",  # Visa/Mastercard debit
            "account_number": r"\b(?:[0-9]{10,20})\b",  # Generic account number
        }
        ...
        try:
            with pdfplumber.open(file_path) as pdf:
                ...
                if len(full_text.strip()) < 100:
                    logging.warning(
                        "Extracted very little text from PDF, parsing may be incomplete"
                    )
                ...
                try:
                    result = self._parse_universal_statement(
                        full_text, pdf, bank_type, file_path
                    )
                    logging.info("Universal parsing completed successfully")
                except Exception as e:
                    logging.error(
                        f"Error in _parse_universal_statement: {str(e)}"
                    )
                    # Create a minimal result to avoid further errors
                    result = {
                        "bank_name": bank_type.capitalize()
                        if bank_type
                        else "Unknown",
                ...
                if result.get("parsing_confidence", 0) < 0.5 or not result.get(
                    "transactions"
                ):
                    ...
                    if bank_type and bank_type in self.bank_patterns:
                        logging.info(
                            f"Universal parsing had low confidence. Trying {bank_type}-specific parsing"
                        )
                        bank_result = self._parse_bank_statement(
                            full_text, bank_type, pdf
                        )

                        # Use bank-specific result if it has higher confidence
                        if bank_result.get(
                            "parsing_confidence", 0
                        ) > result.get("parsing_confidence", 0):
                            result = bank_result
                            logging.info(
                                f"Using {bank_type}-specific parsing results"
                            )
                    else:
                        # Fall back to generic parsing if universal parsing failed and no bank-specific parsing available
                        logging.info(
                            "Universal parsing failed. Using generic parsing as fallback"
                        )
                        generic_result = self._parse_generic_statement(
                            full_text, pdf
                        )

                        # Use generic result if it has higher confidence
                        if generic_result.get(
                            "parsing_confidence", 0
                        ) > result.get("parsing_confidence", 0):
    ...
    def _extract_text_from_pdf(self, pdf) -> str:
        ...
        try:
            ...
            for page in pdf.pages:
                try:
                    ...
                    if page_text:
                        ...
                        logger.warning(
                            f"No text extracted from page {pdf.pages.index(page) + 1}"
                        )
                except Exception as e:
            ...
            if len(full_text.strip()) < 100:
                logger.warning(
                    "Standard PDF extraction yielded minimal results, trying alternative method"
                )
    ...
    def _detect_bank_type(self, text: str, filename: str = "") -> str:
        ...
        for bank, indicators in bank_indicators.items():
            for indicator in indicators:
                ...
                if any(
                    word in indicator or indicator in word
                    for word in text_lower.split()
                ):
                    logger.info(
                        f"Detected bank: {bank} using partial match in text"
                    )
                    return bank
        ...
        section_patterns = {
            ...
            "santander": [
                r"estado de cuenta santander",
                r"resumen de movimientos santander",
            ],
            "banamex": [
                r"estado de cuenta banamex",
                r"resumen de cuenta banamex",
            ],
            "hsbc": [r"estado de cuenta hsbc", r"resumen de cuenta hsbc"],
            "scotiabank": [
                r"estado de cuenta scotia",
                r"resumen de cuenta scotiabank",
            ],
            "banorte": [
                r"estado de cuenta banorte",
                r"resumen de cuenta banorte",
            ],
        }
    ...
    def _parse_bank_statement(
        self, text: str, bank_type: str, pdf
    ) -> Dict[str, Any]:
    ...
    def _parse_universal_statement(
        self, text: str, pdf=None, bank_type: str = None, filename: str = ""
    ) -> Dict[str, Any]:
        ...
        try:
            # Determine bank name
            if bank_type:
                bank_name = self.bank_patterns.get(bank_type.lower(), {}).get(
                    "name", bank_type.capitalize()
                )
            ...
            try:
                ...
                period_result = self._extract_standardized_statement_period(
                    text
                )
                logging.debug(
                    f"Period result type: {type(period_result)}, value: {period_result}"
                )

                if (
                    period_result
                    and isinstance(period_result, tuple)
                    and len(period_result) == 2
                ):
                    start_date, end_date = period_result
                    logging.debug(
                        f"Extracted period: {start_date} to {end_date}"
                    )
                else:
                    logging.debug("Using statement period extraction fallback")
                    # Fallback to enhanced period extraction with bank_type
                    start_date, end_date = (
                        self._extract_statement_period_fallback(text, bank_type)
                    )
                    logging.debug(
                        f"Fallback period result: {start_date} to {end_date}"
                    )
            ...
            try:
                ...
                transactions = self._extract_standardized_transactions(
                    text, bank_type
                )
                logging.debug(
                    f"Transaction extraction result type: {type(transactions)}"
                )
                if transactions is None:
                    logging.warning(
                        "Transaction extraction returned None, using empty list instead"
                    )
                    transactions = []
        ...
        try:
            if bank_type and bank_type.lower() in self.bank_patterns:
                ...
                if not account_holder or account_holder == "Unknown":
                    result["account_holder"] = self._extract_account_holder(
                        text, bank_patterns
                    )

                if not account_number or account_number == "Unknown":
                    result["account_number"] = self._extract_account_number(
                        text, bank_patterns
                    )

                if not (start_date and end_date):
                    start, end = self._extract_statement_period(
                        text, bank_patterns
                    )
                    if start and end:
                ...
                if not transactions and "transaction_pattern" in bank_patterns:
                    try:
                        bank_transactions = self._extract_transactions(
                            text, bank_patterns, bank_type
                        )
                        if bank_transactions and isinstance(
                            bank_transactions, list
                        ):
                            result["transactions"] = bank_transactions
                    except Exception as e:
                        logging.error(
                            f"Error in bank-specific transaction extraction: {str(e)}"
                        )
        except Exception as e:
        ...
        try:
            ...
            if (
                not result["account_holder"]
                or result["account_holder"] == "Unknown"
            ):
                ...
                try:
                    ...
                    logging.warning(
                        "Using BBVA account holder extraction as generic fallback"
                    )
                except Exception as e:
                    logging.error(
                        f"Error in fallback account holder extraction: {str(e)}"
                    )

            if (
                not result["account_number"]
                or result["account_number"] == "Unknown"
            ):
                # Use BBVA method as fallback for any bank
                try:
                    bbva_patterns = self.bank_patterns.get("bbva", {})
                    ...
                    logging.warning(
                        "Using BBVA account number extraction as generic fallback"
                    )
                except Exception as e:
                    logging.error(
                        f"Error in fallback account number extraction: {str(e)}"
                    )
        except Exception as e:
    ...
    def _extract_generic_account_holder(self, text: str) -> Optional[str]:
        """
        Generic extraction of account holder name.
        Looks for a prominent all-caps name (2-5 words) in the first 20 lines.
        """
        lines = text.split("\n")
        # Keywords that might indicate a line is not a person's name
        common_terms_to_avoid = [
            "ESTADO DE CUENTA",
            "CLIENTE",
            "NOMBRE",
            "RFC",
        ...
        # Regex for a line that is all caps, 2-5 words, 8-50 chars long
        # Allows spaces and common accented capital letters in Spanish
        name_pattern = re.compile(r"^[A-ZÁÉÍÓÚÑÜ\s]{8,50}$")

        for line_number, line_text in enumerate(
            lines[:20]
        ):  # Check first 20 lines
            candidate_line = line_text.strip()
            if name_pattern.match(candidate_line):
                words = candidate_line.split()
                if 2 <= len(words) <= 5:
                    # Check if the line contains any of the common terms to avoid
                    contains_avoid_term = False
                    for term in common_terms_to_avoid:
                        if (
                            term in candidate_line
                        ):  # Simple substring check for avoidance
                            contains_avoid_term = True
                            break

                    if not contains_avoid_term:
                        # Additional check for lines that might be headers despite passing filters
                        if not (
                            len(words) == 2 and words[0] in ["SR", "SRA", "LIC"]
                        ):
                            logger.debug(
                                f"Generic account holder candidate on line {line_number + 1}: '{candidate_line}'"
                            )
                            return candidate_line

        logger.warning("Could not generically extract account holder name.")
        return None

    def _extract_generic_account_number(self, text: str) -> Optional[str]:
        """
        Generic extraction of account number.
        Looks for common account number patterns (e.g., 16-digit card, 18-digit CLABE).
        """
        first_n_lines = "\n".join(text.splitlines()[:30])

        # Search in the first N lines
        for name, pattern_regex in self.ACCOUNT_NUMBER_PATTERNS.items():
            match = re.search(pattern_regex, first_n_lines)
            if match:
                account_number = re.sub(
                    r"[ -]", "", match.group(0)
                )  # Remove spaces/hyphens
                logger.info(f"Gen. acct: {account_number} (type: {name})")
                return account_number

        # If not found in first N lines, search the whole text (less reliable)
        for name, pattern_regex in self.ACCOUNT_NUMBER_PATTERNS.items():
            ...
            if match:
                ...
                logger.warning(
                    f"Gen. acct (full): {account_number} (type: {name})"
                )
                return account_number

        logger.warning("Could not generically extract account number.")
        return None
        ...
        if "bbva" in patterns.get("name", "").lower():
            ...
            for line in lines[:15]:
                ...
                if (
                    ...
                    line = re.sub(
                        r"\b(BBVA|BANCOMER|ESTADO|CUENTA|TARJETA|CREDITO)\b",
                        "",
                        line,
                    ).strip()
                    if len(line) > 5:
        ...
        if pattern:
            ...
            if match:
                try:
                    ...
                    logger.debug(f"Period date parse err: {e}")
                    pass

        return None, None

    def _extract_transactions(
        self, text: str, patterns: Dict[str, Any], bank_type: str
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            standardized_transactions = self._extract_standardized_transactions(
                text, bank_type
            )
            if standardized_transactions and isinstance(
                standardized_transactions, list
            ):
                logger.info(
                    f"Extracted {len(standardized_transactions)} transactions using standardized format"
                )
            ...
            if not transaction_patterns:
                logger.warning(
                    f"No transaction patterns defined for {bank_type}"
                )
                return []
    ...
    def _extract_standardized_statement_period(
        self, text: str
    ...
    def _extract_standardized_transactions(
        self, text: str, bank_type: str = None
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logging.debug(
                f"Looking for transaction headers: {transaction_section_headers}"
            )
            ...
            for i, line in enumerate(lines):
                for header in transaction_section_headers:
                    if re.search(header, line, re.IGNORECASE):
                        ...
                        logging.debug(
                            f"Found transaction section start at line {i}: {line}"
                        )
                        break
            ...
            if section_start is None:
                logging.warning(
                    "Could not find transaction section in statement"
                )
                return []
            ...
            if section_end is None:
                ...
                logging.debug(
                    "Could not find transaction section end, using end of text"
                )
            ...
            if section_start is None:
                ...
                if bank_type and bank_type.lower() == "bbva":
                    logging.info(
                        "Trying alternative approach for BBVA - searching for date patterns directly"
                    )
            ...
            logging.debug(
                f"Transaction section length: {len(transaction_section)} characters"
            )
            ...
            if transaction_section:
                ...
                logging.debug(
                    f"Transaction section sample: {transaction_section[:sample_length]}..."
                )
            else:
            ...
            if (
                bank_type
                and bank_type.lower() in self.bank_transaction_patterns
            ):
                patterns = self.bank_transaction_patterns[bank_type.lower()]
                logging.info(
                    f"Using {len(patterns)} {bank_type} transaction patterns"
                )
            ...
            for pattern_idx, pattern in enumerate(patterns):
                matches = list(
                    re.finditer(pattern, transaction_section, re.MULTILINE)
                )
                pattern_match_counts[pattern_idx] = len(matches)
                logging.debug(
                    f"Pattern {pattern_idx} matched {len(matches)} transactions"
                )
                ...
                for match in matches:
                    try:
                        ...
                        if (
                            (pattern_idx in [4, 5, 6])
                            and bank_type
                            and bank_type.lower() == "bbva"
                        ):
                            ...
                            if pattern_idx == 4:  # Credit pattern (with + sign)
                                ...
                                sign = (
                                    match.group("sign")
                                    if "sign" in match.groupdict()
                                    else None
                                )

                                if sign == "+" or (
                                    not sign and "+" in match.string
                                ):
                                    amount_str = "+$" + match.group("amount")
                                    transaction_type = "credit"
                                else:
                            ...
                            if (
                                hasattr(self, "statement_period")
                                and self.statement_period
                            ):
                                try:
                                    if self.statement_period[
                                        0
                                    ]:  # Start date of statement period
                                        statement_month = self.statement_period[
                                            0
                                        ].month
                            ...
                            transactions.append(transaction)
                            logging.debug(
                                f"Extracted transaction: {transaction}"
                            )
                            ...
                            logging.warning(
                                f"Pattern match doesn't have enough groups: {match.groups()}"
                            )
                    except Exception as e:
                        logging.error(f"Error processing match: {str(e)}")
            # Log transaction count
            logging.info(
                f"Extracted {len(transactions)} transactions using standardized format"
            )
            return transactions

        except Exception as e:
            logging.error(
                f"Error in _extract_standardized_transactions: {str(e)}"
            )
            return []
    ...
    def _parse_date(self, date_str: str) -> Optional[datetime]:
        ...
        try:
            ...
            for sp_month, en_month in spanish_months.items():
                date_str = re.sub(
                    rf"\b{sp_month}\b", en_month, date_str, flags=re.IGNORECASE
                )

            # Try parsing with dateutil
            return dateutil.parser.parse(
                date_str, dayfirst=True
            )  # Mexican dates are day-first
        except Exception as e:
    ...
    def _extract_statement_period_fallback(
        self, text: str, bank_type: str = None
    ) -> Tuple[Optional[datetime], Optional[datetime]]:
            ...
            if bank_type and bank_type.lower() == "bbva":
                ...
                for pattern in bbva_patterns:
                    ...
                    if match:
                        ...
                        if start_date and end_date:
                            logging.info(
                                f"Extracted BBVA statement period: {start_date} to {end_date}"
                            )
                            return start_date, end_date

                # Look for year in transaction lines to determine statement year
                year_pattern = (
                    r"-(?P<year>\d{4})\s+(?P<description>.+?)\s+[\+\-]\s+\$"
                )
                            ...
                            if month_match:
                                ...
                                for key, month_num in month_map.items():
                                    if key in month_name:
                                        ...
                                        start_date = datetime(
                                            most_common_year, month_num, 1
                                        )
                                        # End date is last day of month
                                        if month_num == 12:
                                            end_date = datetime(
                                                most_common_year, 12, 31
                                            )
                                        else:
                                            end_date = datetime(
                                                most_common_year,
                                        ...
                                        logging.info(
                                            f"Created approximate BBVA statement period: {start_date} to {end_date}"
                                        )
                                        return start_date, end_date
